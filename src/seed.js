...
export async function seedData() {
  ...
  await db.executeMultiple(`
    INSERT INTO tickets (ticket_number, order_id, caller_type, caller_id, category, description, status, priority, transcript, resolved_by) VALUES
      ('T1001', 5001, 'customer',    6001, 'missing_item',         'Ahmad reports white cheese missing from order 5001',                                                 'open',      'normal', NULL,                                                                          NULL),
      ('T1002', 5002, 'store_staff', 8001, 'inventory_error',      'Milk shows as in-stock in the system but shelf is empty',                                            'resolved',  'high',   NULL,                                                                          'Hanadi Omar'),
      ('T1003', 5003, 'customer',    6003, 'refund_request',       'Mohammed requests refund due to poor chicken quality',                                               'open',      'high',   'Agent: Hello, how can I help?\nCustomer: The chicken was spoiled',           NULL),
      ('T1004', 5001, 'rider',       7001, 'rider_delay',          'Samer stuck in traffic on Jordan Street, 20-minute delay',                                           'open',      'normal', NULL,                                                                          NULL),
      ('T1005', 5003, 'customer',    6003, 'quality_concern',      'Quality complaint\n\nEscalation summary: Customer refuses credit and demands a manager',             'escalated', 'normal', 'Agent: Hello\nCustomer: I want to speak to a manager',                       NULL),
      ...
      ('T1007', 5002, 'customer',    6002, 'wrong_item',           'Sara received yogurt instead of cheese — store credit added as resolution',                          'resolved',  'normal', NULL,                                                                          'ai-agent'),
      ('T1008', 5006, 'rider',       7001, 'customer_unreachable', 'Samer cannot reach the customer at the delivery point for order 5006',                               'open',      'normal', NULL,                                                                          NULL);
  `);
